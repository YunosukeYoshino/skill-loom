...
class SequenceMatcher {
  ...
  private findLongestMatch(
    ...
    while (
      besti > alo &&
      bestj > blo &&
      !this.bjunk.has(this.b[bestj - 1] as string) &&
  ...
  private getMatchingBlocks(): Match[] {
    ...
    blocks.sort(
      (left, right) =>
        left.i - right.i || left.j - right.j || left.size - right.size
    );
    ...
    for (const block of blocks) {
      if (
        current.i + current.size === block.i &&
        current.j + current.size === block.j
      ) {
        current = {
          i: current.i,
          j: current.j,
  ...
  getGroupedOpcodes(n = 3): Opcode[][] {
    ...
    if (
      group.length > 0 &&
      !(group.length === 1 && (group[0] as Opcode)[0] === "equal")
    )
      groups.push(group);
    return groups;
  }
...
export function unifiedDiff(
  ...
  for (const group of new SequenceMatcher(a, b).getGroupedOpcodes(n)) {
    ...
    lines.push(
      `@@ -${formatRange(head[1], tail[2])} +${formatRange(head[3], tail[4])} @@`
    );
