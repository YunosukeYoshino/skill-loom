...
function catalogFile(envName: string, relativePath: string): string {
  ...
  if (process.env.MY_SKILLS_CATALOG_EXPLICIT === "1" && selected) return join(selected, relativePath)
...
export function globalLockFile(): string {
  return envPath("MY_SKILLS_GLOBAL_LOCK_FILE", join(homedir(), ".agents", ".skill-lock.json"))
}
...
export function archiveDir(): string {
  return envPath("MY_SKILLS_ARCHIVE_DIR", join(homedir(), ".agents", "skills-archive"))
}

export function claudeSkillsDir(): string {
  return envPath("MY_SKILLS_CLAUDE_SKILLS_DIR", join(homedir(), ".claude", "skills"))
}

export function geminiSkillsDir(): string {
  return envPath("MY_SKILLS_GEMINI_SKILLS_DIR", join(homedir(), ".gemini", "config", "skills"))
}

export function presetsDir(): string {
  return envPath("MY_SKILLS_PRESETS_DIR", join(homedir(), ".agents", "skill-presets"))
}
...
export function skillsAddScript(): string {
  return envPath("MY_SKILLS_ADD_SCRIPT", join(REPO_LOCAL_SKILLS_DIR, "skills-add", "scripts", "skills-add"))
}
...
export const GLOBAL_INSTALL_AGENTS = ["claude-code", "codex", "antigravity"] as const
