...
import { activeDir, archiveDir, externalCandidatesFile, externalUpdateCheckWorkers, projectDecksDir, skillsAddScript, skillsRemoveBin, skillsUpdateBin } from "./config"
...
export type SourceUpdateStatus = { checked?: boolean; updatable?: string[]; error?: string }
...
export function externalSourceStatusLabel(status: SourceUpdateStatus | undefined): string {
 ...
 */
export async function skillHasRemoteUpdate(name: string, candidate: ExternalCandidate): Promise<boolean> {
...
export function installedExternalUpdateTasks(lock: Lock, sourceFilter?: string | null): UpdateTask[] {
...
function updateSkillPathInLock(name: string, newPath: string): void {
  try {
    ...
    if (lock.external?.[name]) {
      ...
      saveLock(lock)
      console.error(`Auto-healed skillPath for '${name}' to '${newPath}' in lock file`)
    }
  } catch (error) {
    console.error(`Error saving updated skillPath for ${name} to lock: ${error}`)
  }
}

function resolveAndUpdateSkillPath(name: string, source: string): string | null {
  try {
    ...
    console.error(`Failed to auto-heal path for skill '${name}' from '${source}': ${error}`)
  }
  return null
}

export async function checkInstalledSkillRemoteUpdate(task: UpdateTask): Promise<UpdateResult> {
  ...
  try {
    ...
    const localHash = localContentHash(name)
    return [source, name, Boolean(remoteHash && localHash && remoteHash !== localHash), null]
  } catch (error) {
    return [source, name, false, error instanceof Error ? error.message : String(error)]
  }
}

/** 並列度を絞って一斉に投げる。返す順は入力どおり。 */
export async function parallelCheckInstalledSkillUpdates(tasks: UpdateTask[]): Promise<UpdateResult[]> {
  ...
  await Promise.all(
    Array.from({ length: workers }, async () => {
      while (next < tasks.length) {
        const index = next++
        results[index] = await checkInstalledSkillRemoteUpdate(tasks[index] as UpdateTask)
      }
    }),
  )
  return results
...
export async function updatableSkillsForSource(
  ...
  const installed = Object.entries(lock.external ?? {})
    .filter(([name, meta]) => meta.source === ownerRepo && activeExternal.has(name))
    .map(([name]) => name)
...
export async function collectUpdatableSkillNames(
  ...
  if (externalCandidatesFile()) {
    ...
    for (const row of externalSourceSummary(lock)) {
      ...
      if (sourceFilter && source !== normalizeGithubSource(sourceFilter)) continue
      try {
        updatable.push(...(await updatableSkillsForSource(lock, source, externalSkillCandidates(source))))
      } catch (error) {
  ...
  const results = await parallelCheckInstalledSkillUpdates(installedExternalUpdateTasks(lock, sourceFilter))
  const updatable = sortNames(results.filter(([, , hasUpdate, error]) => hasUpdate && !error).map(([, name]) => name))
  const errors = results.filter(([, , , error]) => error).map(([source, name, , error]) => `${source}/${name}: ${error}`)
...
export async function collectExternalUpdateStatus(
  ...
  for (const row of sources) statusBySource[row.source] = { checked: true, updatable: [] }
  ...
  if (externalCandidatesFile()) {
    ...
    for (const row of sources) {
      const source = row.source
      try {
        const updatable = await updatableSkillsForSource(lock, source, externalSkillCandidates(source))
        ;(statusBySource[source] as SourceUpdateStatus).updatable = updatable
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        errors.push(`${source}: ${message}`)
        statusBySource[source] = { checked: false, error: message, updatable: [] }
      }
    }
    for (const status of Object.values(statusBySource)) status.updatable = sortNames(status.updatable ?? [])
  ...
  for (const [source, sourceErrs] of sourceErrors) {
    ...
    if (!status) continue
    const totalForSource = tasks.filter(([taskSource]) => taskSource === source).length
 ...
 */
export async function externalSkillStillUpdatable(lock: Lock, name: string): Promise<boolean> {
  const task = installedExternalUpdateTasks(lock).find(([, taskName]) => taskName === name)
...
export function formatExternalUpdateMessage(
  ...
  if (unchanged.length > 0) parts.push(`未反映(CLI最新扱い): ${unchanged.join(", ")}`)
  if (failed.length > 0) parts.push(`failed: ${failed.join(", ")}`)
  if (fetchErrors.length > 0) parts.push(`確認失敗: ${fetchErrors.slice(0, 3).join("; ")}`)
 ...
 */
export async function runExternalInstall(source: string, selected: Set<string>): Promise<void> {
  ...
  if (exitCode !== 0) {
    throw new Error(`Command '${command.join(" ")}' returned non-zero exit status ${exitCode}.`)
  }
 ...
 */
export function addExternalToLock(source: string, selected: Set<string>, candidates: ExternalCandidate[]): void {
  const ownerRepo = normalizeGithubSource(source)
  const candidateByName = new Map(candidates.map((candidate) => [candidate.name, candidate]))
  const lock = loadLock()
 ...
 */
export function registerInstalledExternalSelection(source: string, selected: Set<string>): [Lock, number] {
  const ownerRepo = normalizeGithubSource(source)
  const lock = loadLock()
  const external = (lock.external ??= {})
  const custom = lock.custom?.skills ?? {}
...
export function removeExternalSkillFromManagement(lock: Lock, skill: string, decksDir = projectDecksDir()): number {
  ...
  for (const entry of entries.filter((name) => name.endsWith(".json")).sort()) {
    ...
    const deck = JSON.parse(readFileSync(path, "utf-8")) as { skills?: string[] }
