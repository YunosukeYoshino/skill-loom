...
function place(name: string, repoBody: string, installedBody: string | null, where: "active" | "archive" = "active"): void {
  writeFile(join(repoFixture, name, "SKILL.md"), repoBody)
  if (installedBody !== null) writeFile(dir(where, name, "SKILL.md"), installedBody)
}

function lockFor(...names: string[]): Lock {
  const skills: Record<string, { repoPath: string; category: string }> = {}
  for (const name of names) {
    skills[name] = { repoPath: `tests/tmp/custom-unit/${name}`, category: "unit" }
  }
...
beforeEach(() => {
  ...
  for (const sub of ["active", "archive", "claude", "gemini"]) mkdirSync(dir(sub), { recursive: true })
...
describe("customSkillRepoDir / installedCustomSkillLocation", () => {
  test("repoPath が無い・実体が無い場合は null", () => {
    place("alpha", "repo\n", "installed\n")
    expect(customSkillRepoDir(lockFor("alpha"), "alpha")).toBe(join(repoFixture, "alpha"))
    expect(customSkillRepoDir(lockFor("alpha"), "missing")).toBeNull()
    expect(customSkillRepoDir({ custom: { skills: { beta: { repoPath: "tests/tmp/nope" } } } }, "beta")).toBeNull()
  })
  ...
  test("active を先に見る。どちらにも無ければ missing", () => {
    ...
    expect(installedCustomSkillLocation("alpha")).toEqual([dir("active", "alpha"), "active"])
    expect(installedCustomSkillLocation("beta")).toEqual([dir("archive", "beta"), "archive"])
  ...
  test("Catalog Root 外の repoPath は拒否する", () => {
    ...
    const lock: Lock = { custom: { skills: { escaped: { repoPath: "../outside" } } } }

    expect(() => customSkillRepoDir(lock, "escaped")).toThrow("Catalog path must stay within Catalog Root")
  })
})
...
describe("skillTreeFileMap", () => {
  test("入れ子も相対パスで平らに並べる。ディレクトリ自体は入らない", () => {
    ...
    expect([...skillTreeFileMap(root).keys()].sort()).toEqual(["SKILL.md", "scripts/run.sh"])
...
describe("skillMdUnifiedDiff", () => {
  test("同じなら空、違えば installed → repo の向きで出す", () => {
    ...
    const diff = skillMdUnifiedDiff(dir("active", "alpha"), join(repoFixture, "alpha"))
    ...
    place("beta", "same\n", "same\n")
    expect(skillMdUnifiedDiff(dir("active", "beta"), join(repoFixture, "beta"))).toBe("")
  })
})
...
describe("customSkillDrift / collectCustomUpdatable", () => {
  ...
  test("差分のあるものだけを名前順で返す", () => {
    ...
    expect(collectCustomUpdatable(lockFor("alpha", "beta", "gamma")).map((row) => row.name)).toEqual(["beta", "gamma"])
  })
})
...
describe("updateCustomFromRepo", () => {
  test("展開先の場所を保ったまま repo の内容で置き換える", () => {
    ...
    expect(updateCustomFromRepo(new Set(["beta", "alpha"]), lockFor("alpha", "beta"))).toEqual(["alpha", "beta"])

    expect(readFileSync(dir("active", "alpha", "SKILL.md"), "utf-8")).toBe("repo alpha\n")
    expect(() => readFileSync(dir("active", "alpha", "stale.md"), "utf-8")).toThrow()
    // archive のものは archive のまま。active へ引き上げると projection が変わる。
    expect(readFileSync(dir("archive", "beta", "SKILL.md"), "utf-8")).toBe("repo beta\n")
    expect(() => readFileSync(dir("active", "beta", "SKILL.md"), "utf-8")).toThrow()
  })
  ...
  test("active なら agent 用の symlink を張り直す", () => {
    ...
    expect(readFileSync(dir("claude", "alpha", "SKILL.md"), "utf-8")).toBe("repo alpha\n")
  })

  test("repo に無い・展開されていないものは例外", () => {
    place("alpha", "repo\n", null)
    expect(() => updateCustomFromRepo(new Set(["alpha"]), lockFor("alpha"))).toThrow("not installed")
    expect(() => updateCustomFromRepo(new Set(["ghost"]), lockFor("ghost"))).toThrow("source not found")
  })
})
