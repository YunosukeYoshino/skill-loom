...
function parseExternalMeta(
  value: unknown,
  path: string
): InventoryExternalSkillMetaV1 {
  const object = objectAt(value, path);
  rejectUnknownKeys(object, EXTERNAL_META_KEYS, path);
  return {
    source: stringAt(required(object, "source", path), `${path}.source`),
    sourceUrl: stringAt(
      required(object, "sourceUrl", path),
      `${path}.sourceUrl`
    ),
    skillPath: stringAt(
      required(object, "skillPath", path),
      `${path}.skillPath`
    ),
    ...optionalMetadata(object, path, [
      "localRepoPath",
      "installSkill",
      "category",
    ]),
...
function parseVendorMeta(
  value: unknown,
  path: string
): InventoryVendorSkillMetaV1 {
  const object = objectAt(value, path);
...
export function parseInventoryLock(
  value: unknown,
  ...
  return {
    ...
    custom: {
      repo: stringAt(
        required(custom, "repo", customPath),
        `${customPath}.repo`
      ),
      skills: recordAt(
        required(custom, "skills", customPath),
        `${customPath}.skills`,
        parseCustomMeta
      ),
    },
    external: recordAt(
      required(root, "external", source),
      `${source}.external`,
      parseExternalMeta
    ),
    vendor: recordAt(
      required(root, "vendor", source),
      `${source}.vendor`,
      parseVendorMeta
    ),
  };
