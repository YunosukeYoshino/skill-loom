...
const lock: Lock = {
  external: {
    gamma: { source: "Owner-Two/repo-two", sourceUrl: "https://github.com/Owner-Two/repo-two.git" },
...
function useCandidates(candidates: { name: string; path?: string; contentHash?: string }[]): void {
...
beforeEach(() => {
  ...
  for (const sub of ["active", "archive", "decks"]) mkdirSync(dir(sub), { recursive: true })
...
describe("externalSourceSummary", () => {
  ...
  test("source が owner/repo の形でなければ owner と repo の両方に入る", () => {
    ...
    expect(row).toMatchObject({ source: "external", owner: "external", repo: "external" })
  })
})
...
describe("externalSourceStatusLabel", () => {
  ...
  test("確認失敗・更新あり・最新を出し分ける", () => {
    ...
    expect(externalSourceStatusLabel({ checked: true, error: "boom" })).toBe("確認失敗")
    expect(externalSourceStatusLabel({ checked: true, updatable: ["alpha", "beta"] })).toBe("更新あり 2")
    expect(externalSourceStatusLabel({ checked: true, updatable: [] })).toBe("最新")
  })
})
...
describe("activeExternalSkillNames / installedExternalUpdateTasks", () => {
  test("archive の skill は active ではないので確認対象に入らない", () => {
    ...
    expect(installedExternalUpdateTasks(lock)).toEqual([["owner-one/repo-one", "alpha", "skills/alpha/SKILL.md"]])
  })
  ...
  test("source を絞れる。URL 形式で渡しても正規化されて一致する", () => {
    ...
    expect(installedExternalUpdateTasks(lock, "https://github.com/owner-one/repo-one").map(([, name]) => name)).toEqual(
      ["beta", "alpha"],
    )
...
describe("collectExternalUpdateStatus", () => {
  test("内容ハッシュが違う active な skill だけ updatable に出る", async () => {
    ...
    place("active", "beta", "local beta\n")
    useCandidates([
      { name: "alpha", path: "skills/alpha/SKILL.md", contentHash: sha256(Buffer.from("remote alpha\n")) },
      { name: "beta", path: "skills/beta/SKILL.md", contentHash: sha256(Buffer.from("local beta\n")) },
    ])

    const [statusBySource, errors] = await collectExternalUpdateStatus(lock)
    expect(errors).toEqual([])
    expect(statusBySource["owner-one/repo-one"]).toEqual({ checked: true, updatable: ["alpha"] })
    // gamma は active ではないので、source ごと「最新」で確定する。
    expect(statusBySource["Owner-Two/repo-two"]).toEqual({ checked: true, updatable: [] })
  })

  test("archive の skill は更新対象にしない", async () => {
    place("archive", "alpha", "local alpha\n")
    useCandidates([{ name: "alpha", path: "skills/alpha/SKILL.md", contentHash: sha256(Buffer.from("remote\n")) }])
...
describe("collectUpdatableSkillNames", () => {
  test("source を跨いで重複なく並ぶ", async () => {
    ...
    useCandidates([
      { name: "alpha", path: "skills/alpha/SKILL.md", contentHash: sha256(Buffer.from("remote alpha\n")) },
      { name: "beta", path: "skills/beta/SKILL.md", contentHash: sha256(Buffer.from("remote beta\n")) },
    ])

    expect(await collectUpdatableSkillNames(lock)).toEqual([["alpha", "beta"], []])
  })
  ...
  test("source を絞ると他の source は確認しない", async () => {
    ...
    expect(await collectUpdatableSkillNames(lock, "Owner-Two/repo-two")).toEqual([[], []])
  })
})
...
describe("外部 CLI のコマンド", () => {
  test("update / remove は -g -y 付きで global に効かせる", () => {
    ...
    expect(externalUpdateCommand("alpha")).toEqual(["/tmp/update-stub", "skills", "update", "alpha", "-g", "-y"])
    expect(externalRemoveCommand("alpha")).toEqual(["/tmp/remove-stub", "skills", "remove", "alpha", "-g", "-y"])
  })
...
describe("formatExternalUpdateMessage", () => {
  ...
  test("確認失敗は 3 件までしか並べない", () => {
    expect(formatExternalUpdateMessage([], [], [], ["1", "2", "3", "4"])).toBe("確認失敗: 1; 2; 3")
  })
})
...
describe("removeExternalSkillFromManagement", () => {
  test("lock からも project deck からも落とし、書き換えた deck の数を返す", () => {
    ...
    writeFileSync(join(decks, "api.json"), JSON.stringify({ skills: ["alpha", "beta"] }))
    writeFileSync(join(decks, "backend.json"), JSON.stringify({ skills: ["gamma"] }))
    mkdirSync(join(decks, "nested"), { recursive: true })
    writeFileSync(join(decks, "nested", "web.json"), JSON.stringify({ skills: ["alpha"] }))
    ...
    expect(JSON.parse(readFileSync(join(decks, "api.json"), "utf-8")).skills).toEqual(["beta"])
    expect(JSON.parse(readFileSync(join(decks, "nested", "web.json"), "utf-8")).skills).toEqual([])
    // 触っていない deck は書き換えない。
    expect(readFileSync(join(decks, "backend.json"), "utf-8")).toBe('{"skills":["gamma"]}')
  })

  test("deck ディレクトリが無くても落ちない", () => {
    const target: Lock = { external: { alpha: {} } }
    expect(removeExternalSkillFromManagement(target, "alpha", dir("no-such-dir"))).toBe(0)
...
describe("addExternalToLock", () => {
  ...
  test("source を正規化して lock に載せる", () => {
    addExternalToLock("https://github.com/Owner/Repo", new Set(["alpha"]), candidates)

    expect(JSON.parse(readFileSync(dir("skills.lock.json"), "utf-8")).external).toEqual({
      alpha: {
        source: "Owner/Repo",
  ...
  test("path の無い候補は既定の置き場所で埋める", () => {
    ...
    expect(JSON.parse(readFileSync(dir("skills.lock.json"), "utf-8")).external.bare.skillPath).toBe(
      "skills/bare/SKILL.md",
    )
  })

  test("custom skill と同名なら external には足さない", () => {
    addExternalToLock("owner/repo", new Set(["mine"]), [{ name: "mine", description: "" }])
  ...
  test("source に無い名前は弾く", () => {
    ...
    expect(() => addExternalToLock("owner/repo", new Set(["nope"]), candidates)).toThrow(
      "Skill not found in source: nope",
    )
  })

  test("取り込んだ skill は ignore から外す", () => {
    writeFileSync(dir("ignore.json"), JSON.stringify({ ignore: ["alpha", "other"] }))
    addExternalToLock("owner/repo", new Set(["alpha"]), candidates)

    expect(JSON.parse(readFileSync(dir("ignore.json"), "utf-8")).ignore).toEqual(["other"])
  })
})
...
describe("registerInstalledExternalSelection", () => {
  ...
  test("CLI lock に取得元が残っていればそちらを写す", () => {
    writeFileSync(
      dir("skill-lock.json"),
      JSON.stringify({
        skills: { alpha: { sourceUrl: "https://example.com/mirror.git", skillPath: "deep/alpha/SKILL.md" } },
      }),
    )

    const [updated, unignored] = registerInstalledExternalSelection("owner/repo", new Set(["alpha", "beta"]))
  ...
  test("ignore を外した件数を返す", () => {
    writeFileSync(dir("ignore.json"), JSON.stringify({ ignore: ["alpha", "beta", "other"] }))

    expect(registerInstalledExternalSelection("owner/repo", new Set(["alpha", "beta"]))[1]).toBe(2)
    expect(JSON.parse(readFileSync(dir("ignore.json"), "utf-8")).ignore).toEqual(["other"])
  })
})
...
describe("externalSourceDetailPayload", () => {
  ...
  test("同じ source に管理対象が残っていれば installed に載り、連続で外せる状態を保つ", async () => {
    const detail = await externalSourceDetailPayload(
      {
        external: {
          alpha: { source: "owner-one/repo-one", skillPath: "skills/alpha/SKILL.md" },
          beta: { source: "owner-one/repo-one", skillPath: "skills/beta/SKILL.md" },
        },
      },
      "owner-one/repo-one",
      [],
    )
    expect(detail.installed.map((skill) => skill.name)).toEqual(["alpha", "beta"])
  })
})
