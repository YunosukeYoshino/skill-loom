...
afterEach(() => {
  ...
  for (const path of tempDirs.splice(0))
    Bun.spawnSync(["/usr/bin/trash", path]);
});
...
describe("resolveCatalogPath", () => {
  test("absolute paths are rejected", () => {
    ...
    expect(() => resolveCatalogPath("/tmp/outside/SKILL.md")).toThrow(
      "Catalog path must be relative"
    );
  });
  ...
  test("parent traversal outside the Catalog Root is rejected", () => {
    ...
    expect(() => resolveCatalogPath("../outside/SKILL.md")).toThrow(
  ...
  test("symlinks that resolve outside the Catalog Root are rejected", () => {
    ...
    expect(() => resolveCatalogPath("escape/SKILL.md")).toThrow(
      "Catalog path must stay within Catalog Root"
    );
  });
});
