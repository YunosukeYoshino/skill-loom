...
describe("installedNames", () => {
  test("symlink 先がディレクトリなら数える（Projection は symlink を張るため）", () => {
    ...
    expect(installedNames(join(sandbox, "active"))).toEqual(
      new Set(["linked"])
    );
  });
...
describe("loadLock", () => {
  test("selected Catalog の不正な Lock をファイル名つきで拒否する", () => {
    ...
    expect(() => loadLock()).toThrow(
      `${path}.version: unsupported version 2; expected 1`
    );
  });
...
describe("frontmatterDescription", () => {
  ...
  test("ブロックスカラーの記号 4 種を受ける", () => {
    for (const marker of [">", "|", ">-", "|-"]) {
      const path = writeSkill(
        join(sandbox, `s-${marker.length}-${marker[0]}`),
        `---\ndescription: ${marker}\n  値\n---\n`
      );
...
describe("skillDescription", () => {
  test("Custom Skill の description は selected Catalog から読む", () => {
    ...
    const lock: Lock = {
      custom: {
        skills: {
          "catalog-skill": { repoPath: "skills/engineering/catalog-skill" },
        },
      },
    };
...
describe("非表示と追跡対象", () => {
  const lock: Lock = {
    ...
    external: {
      "ext-one": { source: "owner/repo" },
      "hidden-ignored": { source: "owner/repo" },
    },
  };
  ...
  test("ignore 指定は tracked から外れ、表示からも隠れる", () => {
    writeFileSync(
      join(sandbox, "ignore.json"),
      JSON.stringify({ ignore: ["hidden-ignored"] })
    );
  ...
  test("ignore ファイルが無ければ ignore 指定なし扱い", () => {
    expect(trackedSkills(lock)).toEqual(
      new Set(["alpha", "ext-one", "hidden-ignored"])
    );
  });
});
