...
beforeEach(() => {
  ...
  writeFileSync(join(sandbox, "shared-decks", "core.json"), JSON.stringify({ name: "core", skills: ["core-skill"] }))
...
describe("saveProjectDeckSelection", () => {
  test("選択を名前順に書き戻し、件数を返す", () => {
    ...
    expect(saveProjectDeckSelection("api", new Set(["gamma", "alpha", "beta"]))).toBe(3)
  ...
  test("親 deck から継承した skill は保存しない", () => {
    ...
    expect(saveProjectDeckSelection("api", new Set([inherited, "own-skill"]))).toBe(1)
...
describe("projectDeckInstallCommands", () => {
  const lock: Lock = {
    custom: { repo: "owner/my-skills", skills: { mine: { repoPath: "skills/a/mine" } } },
    external: {
  ...
  test("source ごとに 1 本へまとめる", () => {
    expect(projectDeckInstallCommands(lock, ["ext", "renamed", "other"])).toEqual([
  ...
  test("custom の repo が無ければ install command を作らない", () => {
    const bare: Lock = { custom: { skills: { mine: { repoPath: "skills/a/mine" } } } }
  ...
  test("シェルに渡せない文字は引用する", () => {
    ...
    expect(projectDeckInstallCommands(odd, ["weird"])[0]).toContain("'a b;rm -rf /'")
  })
})
...
describe("writeProjectDeckSkills", () => {
  test("渡した一覧で skills を差し替え、書いたパスを返す", () => {
    ...
    const path = writeProjectDeckSkills("api", deck, new Set(["gamma", "alpha"]))

    expect(path).toBe(join(sandbox, "api.json"))
    // extends はそのまま残り、skills だけが入れ替わる。
    expect(readDeck("api")).toEqual({ name: "api", extends: ["core"], skills: ["alpha", "gamma"] })
  })
})
...
describe("loadOptionalProjectDeck", () => {
  ...
  test("deck 名があれば親も解決した一覧を返す", () => {
    ...
    expect(loadOptionalProjectDeck("api")[1]).toEqual([...coreSkills, "zz-own"].sort())
  })
})
