...
export function installedCustomSkillLocation(name: string): [string | null, SkillState] {
...
export function skillMdUnifiedDiff(installedDir: string, repoDir: string): string {
  ...
  if (left.length === right.length && left.every((line, index) => line === right[index])) return ""
  return unifiedDiff(
    left,
    right,
...
export function customSkillDrift(lock: Lock, name: string): CustomUpdatable | null {
  ...
  for (const rel of sortNames(new Set([...repoFiles.keys(), ...installedFiles.keys()]))) {
    const repoHash = repoFiles.has(rel) ? skillFileHash(repoFiles.get(rel) as string) : ""
    const installedHash = installedFiles.has(rel) ? skillFileHash(installedFiles.get(rel) as string) : ""
...
export function updateCustomFromRepo(names: Set<string>, lock: Lock): string[] {
  ...
  for (const name of sortNames(names)) {
    const repoDir = customSkillRepoDir(lock, name)
    const [installedDir, state] = installedCustomSkillLocation(name)
    if (repoDir === null) throw new Error(`custom skill source not found: ${name}`)
    if (installedDir === null) throw new Error(`custom skill is not installed: ${name}`)
