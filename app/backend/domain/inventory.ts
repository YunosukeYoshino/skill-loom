...
export function hiddenGlobalSkills(lock: Lock): Set<string> {
  return union(repoLocalSkillNames(), repoLocalCustomSkills(lock), ignoredSkills())
}
...
export function trackedSkills(lock: Lock): Set<string> {
  ...
  const custom = difference(new Set(Object.keys(lock.custom?.skills ?? {})), repoLocalCustomSkills(lock), ignored)
  const external = difference(new Set(Object.keys(lock.external ?? {})), ignored)
...
export function allSkillNames(lock: Lock): Set<string> {
  const names = union(trackedSkills(lock), installedNames(activeDir()), installedNames(archiveDir()))
...
function skillMetadata(ctx: RowContext, name: string): [string, string] {
  ...
  if (name in custom) {
    ...
    return [(typeof meta === "string" ? "" : meta?.category) || "custom", "custom"]
  }
  if (name in external) return [external[name]?.source || "external", "external"]
...
export function skillDescription(lock: Lock, name: string): string {
  ...
  if (repoPath) {
    const description = frontmatterDescription(resolveCatalogPath(`${repoPath}/SKILL.md`))
    if (description) return description
  }

  const localRepoPath = external[name]?.localRepoPath
  if (localRepoPath) {
    const description = frontmatterDescription(resolveCatalogPath(`${localRepoPath}/SKILL.md`))
    if (description) return description
  }
...
export function skillProjectionState(name: string, active: Set<string>, archived: Set<string>): Selection {
...
function tristateSkillRow(ctx: RowContext, name: string): TristateRow {
  ...
  return {
    ...
    selection,
    can_activate: ctx.tracked.has(name) || ctx.active.has(name) || ctx.archived.has(name),
  }
}

export function splitTristateRows(lock: Lock, skillNames: Iterable<string>): [TristateRow[], TristateRow[]] {
  ...
  mainRows.sort((a, b) => {
    const order = (selectionOrder[a.selection] ?? 99) - (selectionOrder[b.selection] ?? 99)
...
export function skillRows(lock: Lock, checked?: Set<string>, names?: Set<string>): SkillRow[] {
  const ctx = rowContext(lock)
  const checkedSet = checked ?? ctx.active
  const nameSet = difference(names ?? allSkillNames(lock), hiddenGlobalSkills(lock))
  ...
  return ordered.map((name) => {
    const [category, source] = skillMetadata(ctx, name)
    const state: SkillState = ctx.active.has(name) ? "active" : ctx.archived.has(name) ? "archive" : "missing"
    return {
      name,
      category,
      description: skillDescription(ctx.lock, name),
