...
export function runSkillsCli(cmd: string[]): void {
  ...
  if (cmd[0] === "bunx" && Bun.which("npx")) {
    ...
    throw new ProjectionInstallError(
      `skills CLI failed (${retry.exitCode}): ${fallback.join(" ")}`
    );
  }
  throw new ProjectionInstallError(
    `skills CLI failed (${result.exitCode}): ${cmd.join(" ")}`
  );
}
...
export function installCustomFromRepo(names: Set<string>, lock: Lock): void {
  ...
  for (const name of sortNames(names)) {
    ...
    if (!exists(src))
      throw new ProjectionInstallError(`custom skill source not found: ${src}`);
...
export function applyDeck(
  ...
  installCustomFromRepo(
    new Set([...install].filter((name) => !external.has(name))),
    lock
  );
...
export function planProjection(
  ...
  return computePresetApplyPlan(
    new Set(intent.target),
    lock,
    intent.touchArchive ?? true
  );
}
...
export function applyPresetTarget(
  ...
  if (plan.unresolved.size > 0 && !skipUnresolved) {
    throw new ValueError(
      `Unresolved: ${sortNames(plan.unresolved).join(", ")}`
    );
  }
...
export function applyNamedPreset(
  name: string,
  lock: Lock,
  backup = true
): PresetPlan {
...
export function bulkOffActive(lock: Lock, backup = true): Set<string> {
  ...
  if (active.size === 0)
    throw new ValueError("No managed active skills to turn off");
  if (backup) backupActiveToLast(lock);
 ...
 */
export function installProjectDeck(
  deckName: string,
  lock: Lock
): ProjectDeckInstallResult {
  ...
  const unresolved = new Set(
    [...target].filter(
      (name) => !managed.has(name) && !active.has(name) && !archived.has(name)
    )
  );
  if (unresolved.size > 0) {
    return {
  ...
  const install = new Set(
    [...target].filter(
      (name) => !active.has(name) && !archived.has(name) && managed.has(name)
    )
  );
