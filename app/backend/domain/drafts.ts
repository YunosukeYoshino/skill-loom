...
import { frontmatterDescription, type Lock, loadLock, saveLock, sortNames } from "./inventory"
import { trashPath } from "./projection"

export type DraftCandidate = { name: string; category: string; repoPath: string; description: string }
...
export function draftSkillMap(): Map<string, DraftCandidate> {
  return new Map(draftSkillCandidates().map((candidate) => [candidate.name, candidate]))
}
 ...
 */
export function promoteDrafts(selected: Set<string>, force = false): [string[], Lock] {
  ...
  if (missing.length > 0) throw new ValueError(`Unknown drafts: ${missing.join(", ")}`)
  ...
  for (const name of promoted) trashPath(resolveCatalogPath((drafts.get(name) as DraftCandidate).repoPath))
