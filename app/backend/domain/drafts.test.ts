...
const mine = (names: string[]) =>
  names.filter((name) => name.startsWith("zz-unit-"));
...
describe("draftSkillCandidates", () => {
  test("frontmatter の name とカテゴリを拾う", () => {
    ...
    const found = draftSkillCandidates().filter(
      (candidate) => candidate.category === CATEGORY_A
    );
  ...
  test("frontmatter に name が無ければディレクトリ名で代用する", () => {
    ...
    expect(
      mine(draftSkillCandidates().map((candidate) => candidate.name))
    ).toEqual(["zz-unit-nameless"]);
  });
  ...
  test("ディレクトリ名として使えない name はディレクトリ名で代用する", () => {
    ...
    expect(
      existsSync(
        catalogPath("skills", CATEGORY_A, "zz-unit-escape", "SKILL.md")
      )
    ).toBe(true);
    expect(existsSync(join(catalogRoot(), "..", "zz-unit-escaped"))).toBe(
      false
    );
  });
  ...
  test("SKILL.md の無いディレクトリは候補にしない", () => {
    created.push(join(draftRoot(), CATEGORY_A));
    mkdirSync(join(draftRoot(), CATEGORY_A, "zz-unit-empty"), {
      recursive: true,
    });
  ...
  test("カテゴリ順・ディレクトリ名順に並ぶ", () => {
    ...
    expect(
      mine(draftSkillCandidates().map((candidate) => candidate.name))
    ).toEqual(["zz-unit-a1", "zz-unit-a2", "zz-unit-b2"]);
  });
});
...
describe("draftRows", () => {
  test("既に custom 登録済みの名前は draft として出さない", () => {
    ...
    const lock = {
      custom: {
        skills: {
          "zz-unit-promoted": { repoPath: "skills/x/y", category: "x" },
        },
      },
    } as Lock;
    const rows = draftRows(lock).filter((row) =>
      row.name.startsWith("zz-unit-")
    );
    expect(rows).toEqual([
      {
        name: "zz-unit-kept",
...
describe("draftConflicts", () => {
  test("custom 登録済みか、配置先が埋まっていれば衝突", () => {
    ...
    expect(draftConflicts(new Set(["a", "b"]), drafts, { b: {} })).toEqual([
      "b",
    ]);
...
describe("promoteDrafts", () => {
  test("コピー・lock 登録・draft 削除まで通す", () => {
    ...
    expect(
      existsSync(catalogPath("skills", CATEGORY_A, "zz-unit-one", "SKILL.md"))
    ).toBe(true);
  ...
  test("未選択と未知の名前は ValueError", () => {
    ...
    expect(() => promoteDrafts(new Set(["zz-unit-missing"]))).toThrow(
      "Unknown drafts: zz-unit-missing"
    );
  });
  ...
  test("配置先が埋まっていれば force 無しでは DraftConflictError", () => {
    ...
    created.push(catalogPath("skills", CATEGORY_A));
    mkdirSync(catalogPath("skills", CATEGORY_A, "zz-unit-one"), {
      recursive: true,
    });
    writeFileSync(
      catalogPath("skills", CATEGORY_A, "zz-unit-one", "SKILL.md"),
      "既存\n"
    );
  ...
  test("force なら配置先を捨てて上書きする", () => {
    ...
    expect(promoteDrafts(new Set(["zz-unit-one"]), true)[0]).toEqual([
      "zz-unit-one",
    ]);
    expect(
      readFileSync(
