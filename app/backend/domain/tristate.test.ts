...
import { computeTristateApplyDelta, formatTristateApplySummary, parseTristateStates } from "./tristate"

let sandbox: string
const touched: string[] = []

const lock: Lock = {
  custom: { repo: "owner/catalog", skills: { alpha: { repoPath: "skills/a/alpha" } } },
...
function place(where: "active" | "archive", ...names: string[]): void {
  for (const name of names) mkdirSync(join(sandbox, where, name), { recursive: true })
}
...
describe("parseTristateStates", () => {
  test("tristate として妥当な値だけ拾う", () => {
    const states = parseTristateStates({ states: { a: "active", b: "off", c: "archive", d: "bogus", e: 1 } })
...
describe("computeTristateApplyDelta", () => {
  test("現状と同じ指定は何も生まない", () => {
    ...
    expect(delta({ alpha: "active", ext: "archive" })).toEqual({
      extra: [], restore: [], install: [], remove: [], unresolved: [],
    })
  })
  ...
  test("active を archive にすると extra", () => {
    place("active", "alpha")
    expect(delta({ alpha: "archive" })).toMatchObject({ extra: ["alpha"], install: [] })
  })
...
describe("formatTristateApplySummary", () => {
  ...
  test("何も無ければ変更なし", () => {
    expect(formatTristateApplySummary(s(), s(), s(), s())).toBe("変更はありません")
  })
