...
describe("サンドボックス差し替え", () => {
  ...
  test("差し替えないとホーム配下を指す（テストが偽陽性でないことの確認）", () => {
    ...
    expect(config.geminiSkillsDir()).toBe(join(home, ".gemini", "config", "skills"))
    expect(config.presetsDir()).toBe(join(home, ".agents", "skill-presets"))
    expect(config.globalLockFile()).toBe(join(home, ".agents", ".skill-lock.json"))
  })
  ...
  test("リポジトリ相対の既定値がリポジトリ内を指す", () => {
    ...
    expect(config.ignoreFile()).toBe(join(config.REPO_ROOT, ".skills-ignore.json"))
    expect(config.projectDecksDir()).toBe(join(config.REPO_ROOT, "project-decks"))
  })
