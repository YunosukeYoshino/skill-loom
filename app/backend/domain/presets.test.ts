...
const lock: Lock = {
  custom: {
    repo: "owner/catalog",
    skills: {
      keep: { repoPath: "skills/a/keep" },
      drop: { repoPath: "skills/a/drop" },
    },
  },
  external: {},
};
...
beforeEach(() => {
  ...
  for (const sub of ["active", "archive", "presets"])
...
describe("validatePresetName", () => {
  test("`_last` はユーザーからは指定させない", () => {
    expect(() => validatePresetName("_last")).toThrow(
      "Reserved preset name: _last"
    );
...
describe("loadPreset", () => {
  ...
  test("壊れた JSON も skills の型崩れも ValueError", () => {
    ...
    writeFileSync(
      dir("presets", "typed.json"),
      JSON.stringify({ name: "typed", skills: [1, 2] })
    );
...
describe("savePresetFromActive", () => {
  test("active が空なら保存しない（戻せない preset を作らない）", () => {
    expect(() => savePresetFromActive("daily", lock)).toThrow(
      "Cannot save preset: no active skills"
    );
  });
  ...
  test("保存され、description が空ならキーごと省かれる", () => {
    ...
    expect(readFileSync(dir("presets", "daily.json"), "utf-8")).not.toContain(
      "description"
    );
  ...
  test("同名は overwrite が無ければ AlreadyExistsError（409 の材料）", () => {
    ...
    expect(() =>
      savePresetFromActive("daily", lock, "二度目", true)
    ).not.toThrow();
...
describe("computePresetApplyPlan", () => {
  ...
  test("lock にもディスクにも無い名前は unresolved", () => {
    expect(
      sorted(
        computePresetApplyPlan(new Set(["missing-skill"]), lock).unresolved
      )
...
describe("preview", () => {
  test("plan は active / off / install / unresolved に並べ替えられる", () => {
    ...
    const preview = presetPlanPreview(
      computePresetApplyPlan(new Set(["keep", "ghost"]), lock)
    );
  ...
  test("変化が無ければ「変更はありません」", () => {
    place("active", "keep");
    expect(
      formatPresetApplyPreview(computePresetApplyPlan(new Set(["keep"]), lock))
    ).toBe("変更はありません");
  });
  ...
  test("skill が 1 つも入っていない preset は当てられない", () => {
    ...
    expect(() => previewNamedPreset("empty", lock)).toThrow(
      "Cannot apply preset: no skills in preset"
    );
  });
});
...
describe("previewRestorePrevious", () => {
  test("`_last` が無ければ ValueError", () => {
    expect(() => previewRestorePrevious(lock)).toThrow(
