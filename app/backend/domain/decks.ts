...
export type Deck = { name?: string; description?: string; extends?: string[]; skills?: string[] }
...
export function listProjectDecks(): string[] {
  ...
  return findJsonFiles(root).map((path) => relative(root, path).replace(/\.json$/, ""))
}
...
export function deckPath(name: string, project = false): string {
  ...
  throw new UnknownDeckError(`Unknown ${project ? "project deck" : "deck"}: ${name}`)
}

/** `extends` を辿って skill 名を集める。循環は移行前と同じくエラーにする。 */
export function loadDeck(name: string, seen: Set<string> = new Set(), project = false): [Deck, string[]] {
  const seenKey = `${project ? "project" : "deck"}:${name}`
  if (seen.has(seenKey)) throw new UnknownDeckError(`Deck cycle detected: ${name}`)
  const nextSeen = new Set(seen)
  nextSeen.add(seenKey)

  const deck = JSON.parse(readFileSync(deckPath(name, project), "utf-8")) as Deck
 ...
 */
export function saveProjectDeckSelection(deckName: string, selected: Set<string>): number {
  ...
  deck.skills = [...selected].filter((name) => !inherited.has(name)).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))
  const tmp = `${path}.tmp`
 ...
 */
export function addSkillsToProjectDeck(deckName: string, selected: Set<string>): number {
  const path = deckPath(deckName, true)
  const deck = JSON.parse(readFileSync(path, "utf-8")) as Deck
  deck.skills = [...new Set([...(deck.skills ?? []), ...selected])].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))
  const tmp = `${path}.tmp`
  writeFileSync(tmp, `${JSON.stringify(deck, null, 2)}\n`)
  renameSync(tmp, path)
  return deck.skills.length
 ...
 */
export function writeProjectDeckSkills(deckName: string, deck: Deck, skills: Set<string>): string {
  const path = deckPath(deckName, true)
 ...
 */
export function projectDeckInstallCommands(lock: Lock, skills: string[]): string[] {
