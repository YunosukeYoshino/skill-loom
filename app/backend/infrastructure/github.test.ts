...
describe("githubRawSkillUrl", () => {
  test("HEAD 固定の raw URL を組み立てる", () => {
    ...
    expect(
      githubRawSkillUrl("owner-one/repo-one", "skills/alpha/SKILL.md")
    ).toBe(
...
describe("findCliSkillMdPaths", () => {
  ...
  test("prefix 配下の直下と 1 階層下を拾う", () => {
    const tree = [
      ...
      blob("skills/beta/SKILL.md"),
      blob("README.md"),
    ];
    expect(findCliSkillMdPaths(tree)).toEqual([
  ...
  test("node_modules などは 2 階層下の対象から外す", () => {
    ...
    const tree = [
      blob("skills/alpha/SKILL.md"),
      blob("skills/node_modules/pkg/SKILL.md"),
    ];
