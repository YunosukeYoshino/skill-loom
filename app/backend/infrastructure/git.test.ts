...
beforeEach(() => {
  ...
  expect(git(["config", "user.email", "skill-loom@example.invalid"]).code).toBe(
    0
  );
});
...
describe("repoRelativePaths", () => {
  test("paths are relative to the selected Catalog Root", () => {
    expect(repoRelativePaths([join(catalog, "skills.lock.json")])).toEqual([
      "skills.lock.json",
    ]);
  });
...
describe("commitRepoChanges", () => {
  test("automatic commits are created in the selected Catalog repository", () => {
    ...
    expect(commitRepoChanges("test: update catalog lock", [lockPath])).toBe(
      " / git commit 済み"
    );

    expect(
      git(["log", "--oneline", "--", "skills.lock.json"]).stdout
    ).toContain("test: update catalog lock");
