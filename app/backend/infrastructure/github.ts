...
import {
  CLI_SKILL_MD_PRIORITY_PREFIXES,
  externalCandidatesFile,
  remoteSkillFetchTimeout,
} from "../domain/config";
...
export function normalizeGithubSource(source: string): string {
  ...
  if (!match?.[1])
    throw new ValueError(`GitHub URL or owner/repo expected: ${source}`);
  return match[1];
}

export function githubRawSkillUrl(
  ownerRepo: string,
  skillPath: string
): string {
...
export async function fetchGithubRepoTree(
  ...
  const payload = (await fetchJson(url, githubAuthHeaders())) as {
    tree?: unknown;
  };
...
export function findCliSkillMdPaths(tree: TreeEntry[], subpath = ""): string[] {
  ...
  const filtered = prefix
    ? allSkillMds.filter(
        (path) => path.startsWith(prefix) || path === `${prefix}SKILL.md`
      )
    : allSkillMds;
 ...
 */
export async function cliSkillNeedsUpdate(
  name: string
...
export function uniqueExternalSkillCandidates(
...
export function discoverExternalSkillCandidates(
  repoDir: string
): ExternalCandidate[] {
...
export function externalSkillCandidates(source: string): ExternalCandidate[] {
  ...
  if (fixture) {
    return uniqueExternalSkillCandidates(
      JSON.parse(readFileSync(fixture, "utf-8")) as ExternalCandidate[]
    );
  }
  const ownerRepo = normalizeGithubSource(source);
  ...
  try {
    ...
    if (clone.exitCode !== 0) {
      throw new ValueError(
        `git clone failed: ${clone.stderr.toString().trim() || ownerRepo}`
      );
    }
