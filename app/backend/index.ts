...
import { activeDir, archiveDir, ignoreFile, lockFile, projectDecksDir } from "./domain/config"
import { resolveCatalogPath } from "./domain/catalogPaths"
import { collectCustomUpdatable, installedCustomSkillLocation, updateCustomFromRepo } from "./domain/custom"
import {
...
import { externalSkillCandidates, normalizeGithubSource } from "./infrastructure/github"
import {
  ignoredSkills,
  loadLock,
  saveLock,
...
import { computeTristateApplyDelta, formatTristateApplySummary, parseTristateStates } from "./domain/tristate"
...
async function waitForPort(hostname: string, port: number, timeoutMs = 20_000): Promise<boolean> {
...
async function serveStaticPublicFile(name: string, contentType: string): Promise<Response> {
...
type WsData = { target: string; protocol: string | null; upstream?: WebSocket; queue: WsFrame[] }
...
function errorResponse(message: string, status: number, payload: object): Response {
...
app.post("/api/apply", async (c) => {
  ...
  if (Object.keys(states).length === 0) return errorResponse("変更がありません", 400, base)

  const { extra, restore, install, remove, unresolved } = computeTristateApplyDelta(states, loadLock())
  // 解決できない skill が 1 つでもあれば、1 バイトも書かずに返す。
  if (unresolved.size > 0) {
    return errorResponse(`Unresolved: ${sortNames(unresolved).join(", ")}`, 400, base)
  }
  ...
  try {
    ...
    warning = applyDeck(extra, restore, new Set([...install].filter((n) => tracked.has(n))), loadLock(), remove)
  } catch (error) {
    return errorResponse(`Apply failed: ${errorText(error)}`, 500, globalPayload(loadLock(), ""))
  } finally {
    releaseApply()
  }
...
app.post("/api/bulk-off", () => {
  ...
  try {
    ...
    if (error instanceof ValueError) return errorResponse(error.message, 400, base)
    return errorResponse(`Bulk off failed: ${errorText(error)}`, 500, globalPayload(loadLock(), ""))
  } finally {
    releaseApply()
  }
...
app.get("/api/presets/:name/preview", (c) => {
  const base = globalPayload(loadLock(), "")
  try {
    return jsonResponse({ ...base, presetPreview: previewNamedPreset(c.req.param("name"), loadLock()) }, 200)
  } catch (error) {
...
app.post("/api/presets/save", async (c) => {
  ...
  try {
    saved = savePresetFromActive(name, loadLock(), bodyString(body, "description"), bodyFlag(body, "overwrite"))
  } catch (error) {
    // 既存 preset の上書き拒否だけは 409。フロントはこれを見て overwrite を出す。
    if (error instanceof AlreadyExistsError) return errorResponse(error.message, 409, base)
    return errorResponse(errorText(error), 400, base)
  }
  return jsonResponse(globalPayload(loadLock(), `Saved preset: ${saved.name} (${saved.skills.length} skills)`), 200)
})
...
app.post("/api/presets/apply", async (c) => {
  ...
  if (!bodyFlag(body, "confirm")) {
    return jsonResponse({ ...base, presetPreview: preview, message: "確認してください" }, 200)
  }
  if (!tryAcquireApply()) return errorResponse(APPLY_BUSY_MESSAGE, 409, base)
  ...
  return jsonResponse(globalPayload(loadLock(), `Applied preset: ${name}`), 200)
})
...
app.post("/api/presets/restore", async (c) => {
  const body = await readJson(c.req.raw)
  const base = globalPayload(loadLock(), "")
  if (!hasPreviousPreset()) return errorResponse("No previous state saved", 400, base)
  ...
  if (!bodyFlag(body, "confirm")) {
    ...
    const message =
      skipped.length > 0 ? `確認してください（復元スキップ ${skipped.length}: ${skipped.join(", ")}）` : "確認してください"
  ...
  try {
    ...
    return errorResponse(`Restore failed: ${errorText(error)}`, 500, globalPayload(loadLock(), ""))
  } finally {
    releaseApply()
  }
  ...
  if (skipped.length > 0) message += `（スキップ ${skipped.length}: ${skipped.join(", ")}）`
  return jsonResponse(globalPayload(loadLock(), message), 200)
})
...
app.post("/api/presets/delete", async (c) => {
  ...
  return jsonResponse(globalPayload(loadLock(), `Deleted preset: ${name}`), 200)
})

// Apply とはロックを共有するが、文言は移行前のまま操作ごとに分けている。
// update の文言は custom と外部 source で同じなので 1 つで足りる。
const UPDATE_BUSY_MESSAGE = "Update already running. Wait for the current update to finish."
const REMOVE_BUSY_MESSAGE = "Remove already running. Wait for the current remove to finish."
...
function checkedPayload(message: string): Response {
  const lock = loadLock()
  return jsonResponse(
    globalPayload(lock, message, { customUpdatesChecked: true, customUpdatable: collectCustomUpdatable(lock) }),
    200,
  )
}
...
app.post("/api/custom/check-updates", () => {
  ...
  const message = updatable.length > 0 ? `更新確認完了: 更新あり ${updatable.length} skills` : "更新確認完了: すべて最新です"
...
app.post("/api/custom/update", async (c) => {
  ...
  const base = globalPayload(loadLock(), "")
  if (!skill) return errorResponse("updateするskillを選択してください", 400, base)
  ...
  if (installedCustomSkillLocation(skill)[0] === null) {
    return errorResponse(`展開されていないため update できません: ${skill}`, 400, base)
  }
  ...
  try {
    ...
    return errorResponse(`updateに失敗: ${errorText(error)}`, 500, globalPayload(loadLock(), ""))
  } finally {
    releaseApply()
  }
...
app.post("/api/custom/update-all", async (c) => {
  ...
  const base = globalPayload(loadLock(), "")
  if (names.size === 0) return errorResponse("更新対象のskillはありません", 400, base)
  const custom = loadLock().custom?.skills ?? {}
  const unknown = sortNames([...names].filter((name) => !(name in custom)))
  if (unknown.length > 0) return errorResponse(`custom skill ではありません: ${unknown.join(", ")}`, 400, base)
  if (!tryAcquireApply()) return errorResponse(UPDATE_BUSY_MESSAGE, 409, base)
...
function runExternalCommand(command: string[]): void {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Command '${command.join(" ")}' returned non-zero exit status ${result.exitCode}.`)
  }
}

app.get("/api/external-sources", () => jsonResponse(externalSourcesPayload(loadLock()), 200))
...
app.get("/api/ogp/:source{.+}", async (c) => {
  ...
  try {
    ownerRepo = normalizeGithubSource(c.req.param("source").replace(/^\/+|\/+$/g, ""))
  } catch (error) {
    return jsonResponse({ message: errorText(error) }, 400)
  }
  try {
    return jsonResponse({ ...(await fetchOgp(ownerRepo)), source: ownerRepo }, 200)
  } catch (error) {
...
app.post("/api/external-sources/check-all-updates", async () => {
  const [statusBySource, errors] = await collectExternalUpdateStatus(loadLock())
  const total = Object.values(statusBySource).reduce(
    (sum, status) => sum + (status.checked ? (status.updatable?.length ?? 0) : 0),
    0,
  )
  let message: string
  if (errors.length > 0) message = `一部のsource確認に失敗 (${errors.length}): ${errors.slice(0, 3).join("; ")}`
  else if (total) message = `更新確認完了: 更新あり ${total} skills`
  else message = "更新確認完了: すべて最新です"
  return jsonResponse(externalSourcesPayload(loadLock(), message, statusBySource), 200)
})
...
app.post("/api/external-sources/update", async (c) => {
  ...
  if (!activeExternalSkillNames(loadLock()).has(skill)) {
    return errorResponse(`active ではないため update できません: ${skill}`, 400, base)
  }
  if (!tryAcquireApply()) return errorResponse(UPDATE_BUSY_MESSAGE, 409, base)
  ...
  try {
    ...
    return errorResponse(`updateに失敗: ${errorText(error)}`, 500, externalSourcesPayload(loadLock()))
  } finally {
    releaseApply()
  }
...
app.post("/api/external-sources/update-all", async (c) => {
  ...
  const [updatable, fetchErrors] = await collectUpdatableSkillNames(loadLock(), sourceFilter)
  ...
  if (!tryAcquireApply()) return await updateAllErrorResponse(sourceFilter, UPDATE_BUSY_MESSAGE, 409)
  ...
  try {
    for (const skill of updatable) {
      try {
        runExternalCommand(externalUpdateCommand(skill))
        if (await externalSkillStillUpdatable(loadLock(), skill)) unchanged.push(skill)
  ...
  const message = formatExternalUpdateMessage(updated, unchanged, failed, fetchErrors)
  const status = failed.length > 0 ? 500 : 200
  if (sourceFilter) {
    try {
      const candidates = externalSkillCandidates(sourceFilter)
      return jsonResponse(await externalSourceDetailPayload(loadLock(), sourceFilter, candidates, message), status)
    } catch (error) {
      return errorResponse(`${message} / 再確認失敗: ${errorText(error)}`, 500, externalSourcesPayload(loadLock()))
    }
  }
  const [statusBySource] = await collectExternalUpdateStatus(loadLock())
  return jsonResponse(externalSourcesPayload(loadLock(), message, statusBySource), status)
})
...
app.post("/api/external-sources/remove", async (c) => {
  const skill = bodyString(await readJson(c.req.raw), "skill")
  const base = externalSourcesPayload(loadLock())
  if (!skill) return errorResponse("管理から外すskillを選択してください", 400, base)
  if (!isArgvSafeSkillName(skill)) return errorResponse(`skill名として扱えません: ${skill}`, 400, base)
  ...
  try {
    ...
    commitNote = commitRepoChanges(`chore: remove ${skill} from skills.lock.json`, [lockFile(), projectDecksDir()])
  } catch (error) {
    return errorResponse(`removeに失敗: ${errorText(error)}`, 500, externalSourcesPayload(loadLock()))
  } finally {
    releaseApply()
  }
...
app.get("/api/external-sources/:source{.+}", async (c) => {
  return await sourceDetailResponse(c.req.param("source").replace(/^\/+|\/+$/g, ""), 200)
})

/** 候補取得に失敗したときは source 一覧の payload へ落として、画面を描けるようにする。 */
async function sourceDetailResponse(source: string, status: number): Promise<Response> {
  let candidates: ReturnType<typeof externalSkillCandidates>
  try {
    candidates = externalSkillCandidates(source)
  } catch (error) {
    return errorResponse(`候補取得に失敗: ${errorText(error)}`, 400, externalSourcesPayload(loadLock()))
  }
  return jsonResponse(await externalSourceDetailPayload(loadLock(), source, candidates), status)
}
...
async function updateAllErrorResponse(
  ...
  if (!sourceFilter) return errorResponse(message, status, externalSourcesPayload(loadLock()))
  let candidates: ReturnType<typeof externalSkillCandidates>
  try {
    candidates = externalSkillCandidates(sourceFilter)
  } catch (error) {
    return errorResponse(`候補取得に失敗: ${errorText(error)}`, status, externalSourcesPayload(loadLock()))
  }
  const detail = await externalSourceDetailPayload(loadLock(), sourceFilter, candidates)
  return errorResponse(message, status, detail)
}

// ---- draft（#72 で移植）----

const DRAFT_ACTIONS: ReadonlySet<string> = new Set(["promote", "install", "promote-force", "install-force"])
const DRAFT_BUSY_MESSAGE = "Draft operation already running. Wait for the current operation to finish."
...
app.post("/api/drafts/:action", async (c) => {
  ...
  if (!DRAFT_ACTIONS.has(action)) return jsonResponse({ message: "Not found" }, 404)

  const body = await readJson(c.req.raw)
  const requested = (body as Record<string, unknown> | null)?.skills
  const selected = new Set(Array.isArray(requested) ? requested.map((name) => String(name)) : [])
  const install = action === "install" || action === "install-force"
  const force = action === "promote-force" || action === "install-force"

  if (!tryAcquireApply()) return errorResponse(DRAFT_BUSY_MESSAGE, 409, draftsPayload(loadLock()))
  try {
    ...
    for (const name of promoted) {
      const meta = newLock.custom?.skills?.[name] as { repoPath: string; category: string }
      paths.push(resolveCatalogPath(meta.repoPath), resolveCatalogPath(`drafts/skills/${meta.category}/${name}`))
    }
    const commitNote = commitRepoChanges(`feat: add ${promoted.join(", ")}`, paths)
    if (install) installCustomFromRepo(new Set(promoted), loadLock())
    const label = install ? "draft解除してglobalに追加" : "draft解除"
    return jsonResponse(draftsPayload(loadLock(), `${label}: ${promoted.join(", ")}${commitNote}`), 200)
  } catch (error) {
    ...
    return errorResponse(`Draft operation failed: ${errorText(error)}`, 400, draftsPayload(loadLock()))
  } finally {
    releaseApply()
  }
})
...
app.get("/api/project-decks/:deckName", (c) => {
  ...
  const showCatalog = c.req.query("catalog") === "1"
  try {
    return jsonResponse(projectDeckPayload(loadLock(), deckName, "", { showCatalog }), 200)
  } catch (error) {
...
app.post("/api/project-decks/:deckName/:action", async (c) => {
  ...
  const action = c.req.param("action")
  // 移行前は未知の action にルーティング自体が当たらず 404 だった。
  if (!DECK_ACTIONS.has(action)) return jsonResponse({ message: "Not found" }, 404)
  ...
  if (action === "save") {
    ...
    try {
      ...
      return errorResponse(`Save failed: ${errorText(error)}`, 500, projectDeckPayload(loadLock(), deckName))
    }
    return jsonResponse(projectDeckPayload(loadLock(), deckName, `Saved deck: ${savedCount} direct skills`), 200)
  }
  ...
  const target =
    action === "merge" ? new Set([...active, ...selected]) : new Set([...loadDeck("core")[1], ...selected])

  const managed = trackedSkills(lock)
  const unmanaged = ignoredSkills()
  const unresolved = sortNames(
    [...target].filter(
      (name) => !managed.has(name) && !unmanaged.has(name) && !active.has(name) && !archived.has(name),
    ),
  )
  const base = projectDeckPayload(lock, deckName)
  if (unresolved.length > 0) return errorResponse(`Unresolved: ${unresolved.join(", ")}`, 400, base)
  ...
  const install = new Set(
    [...target].filter((name) => !active.has(name) && !archived.has(name) && !unmanaged.has(name) && managed.has(name)),
  )
  if (!tryAcquireApply()) return errorResponse(APPLY_BUSY_MESSAGE, 409, base)
  ...
  try {
    ...
    return errorResponse(`Apply failed: ${errorText(error)}`, 500, projectDeckPayload(loadLock(), deckName))
  } finally {
    releaseApply()
  }
  return jsonResponse(projectDeckPayload(loadLock(), deckName, `Applied: active target ${target.size}`), 200)
})

// ---- 外部 skill の取り込み（#74 で移植）----

const IMPORT_BUSY_MESSAGE = "Import already running. Wait for the current import to finish."
...
app.post("/api/external/preview", async (c) => {
  ...
  try {
    ...
    return errorResponse(`候補取得に失敗: ${errorText(error)}`, 400, catalogPayload(deckName))
  }
  if (candidates.length === 0) {
    return errorResponse("SKILL.md が見つかりませんでした", 404, catalogPayload(deckName))
  }
  return jsonResponse(externalPreviewPayload(loadLock(), deckName, source, candidates), 200)
})
...
app.post("/api/external/install", async (c) => {
  ...
  if (selected.size === 0) {
    return errorResponse("取り込むskillを選択してください", 400, catalogPayload(deckName))
  }
  if (!tryAcquireApply()) return errorResponse(IMPORT_BUSY_MESSAGE, 409, catalogPayload(deckName))
  try {
    await runExternalInstall(ownerRepo, selected)
  } catch (error) {
    return errorResponse(`取り込みに失敗: ${errorText(error)}`, 500, catalogPayload(deckName))
  } finally {
    releaseApply()
  }

  const names = sortNames(selected).join(", ")
  const [, unignoredCount] = registerInstalledExternalSelection(ownerRepo, selected)
  const unignoredMessage = unignoredCount ? ` / ignored解除 ${unignoredCount}` : ""
  const commitNote = commitRepoChanges(`chore: add ${names} to skills.lock.json`, [lockFile(), ignoreFile()])

  if (!deckName) {
    return jsonResponse(
      globalPayload(loadLock(), `外部skillsをglobalに追加しました: ${names}${unignoredMessage}${commitNote}`, {
        showCatalog: true,
      }),
      200,
    )
  }
  const path = writeProjectDeckSkills(deckName, deck, new Set([...currentSkills, ...selected]))
  const deckCommitNote = commitRepoChanges(`chore: add ${names} to project deck ${deckName}`, [path])
  return jsonResponse(
    projectDeckPayload(
      loadLock(),
      deckName,
...
app.post("/api/external/add-to-deck", async (c) => {
  ...
  if (selected.size === 0) {
    return errorResponse("deckに追加するskillを選択してください", 400, catalogPayload(deckName))
  }
  if (!tryAcquireApply()) return errorResponse(IMPORT_BUSY_MESSAGE, 409, catalogPayload(deckName))
  ...
  try {
    ...
    return errorResponse(`deck追加に失敗: ${errorText(error)}`, 500, catalogPayload(deckName))
  } finally {
    releaseApply()
  }

  const names = sortNames(selected).join(", ")
  const commitNote = commitRepoChanges(`chore: add ${names} to skills.lock.json and deck ${deckName}`, [
    lockFile(),
    deckPath(deckName, true),
  ])
  return jsonResponse(
    projectDeckPayload(loadLock(), deckName, `deckに追加しました: ${names} (${savedCount} skills)${commitNote}`, {
      showCatalog: true,
    }),
    200,
  )
...
app.get("/favicon.ico", () => serveStaticPublicFile("favicon.ico", "image/x-icon"))
app.get("/favicon.svg", () => serveStaticPublicFile("favicon.svg", "image/svg+xml"))
app.get("/apple-touch-icon.png", () => serveStaticPublicFile("apple-touch-icon.png", "image/png"))
app.get("/site.webmanifest", () => serveStaticPublicFile("site.webmanifest", "application/manifest+json"))
...
app.on(["GET", "HEAD"], "*", async (c) => {
  ...
  const upstream = await fetch(target, { method: c.req.method, headers, redirect: "manual" })
...
app.all("*", () => jsonResponse({ detail: "Method Not Allowed" }, 405, { allow: "HEAD, GET" }))
...
const server = Bun.serve({
  ...
  fetch(req, srv) {
    ...
    if (vitePort !== null && req.headers.get("upgrade")?.toLowerCase() === "websocket") {
