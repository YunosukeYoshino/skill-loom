...
const SKILLS_RESTORE = path.join(
  REPO_ROOT,
  ".agents/skills/skills-restore/scripts/skills-restore"
);
const SKILLS_ADD_DIR = path.join(
  REPO_ROOT,
  ".agents/skills/skills-add/scripts"
);
const VENDOR_FORK = path.join(
  REPO_ROOT,
  ".agents/skills/vendor-fork/scripts/skills-vendor-fork"
);
const VENDOR_AUDIT = path.join(
  REPO_ROOT,
  ".agents/skills/vendor-fork/scripts/skills-audit-vendor"
);
const VENDOR_SYNC = path.join(
  REPO_ROOT,
  ".agents/skills/vendor-fork/scripts/skills-sync-upstream"
);

const SKILLS_ADD_SCRIPTS = [
...
function copySkillsAddScripts(repo: string): void {
  for (const rel of SKILLS_ADD_SCRIPTS) {
    fs.copyFileSync(path.join(REPO_ROOT, rel), path.join(repo, rel));
  }
  const skillMdRel = "app/backend/domain/skillMd.ts";
  fs.mkdirSync(path.dirname(path.join(repo, skillMdRel)), { recursive: true });
  fs.copyFileSync(
    path.join(REPO_ROOT, skillMdRel),
    path.join(repo, skillMdRel)
  );
}
  ...
  test("public 管理スキルは Catalog 名のつかない一般 api を指し、private repo / git push を含まない", () => {
    ...
    for (const name of MANAGEMENT_SKILLS) {
      const text = fs.readFileSync(
        path.join(REPO_ROOT, ".agents/skills", name, "SKILL.md"),
        "utf-8"
      );
      expect(
        text.includes("CATALOG_ROOT") || text.includes("MY_SKILLS_CATALOG_DIR")
      ).toBe(true);
...
describe("skills-restore", () => {
  test("dry-run で selected catalog の外部/custom/agents を plan する", () => {
    ...
    fs.writeFileSync(
      path.join(catalog, "agents", "catalog-agent.md"),
      "catalog agent\n"
    );
    fs.writeFileSync(
    ...
    const out = runBash(
      SKILLS_RESTORE,
      ["--catalog-dir", catalog, "--dry-run"],
      { env: { HOME: path.join(root, "home") } }
    );
    expect(out.exitCode).toBe(0);
    expect(out.stdout).toContain(
      "npx skills add owner/external --skill external"
    );
...
describe("skills-vendor-fork", () => {
  test("selected catalog が vendor と upstream の実体を保有する", () => {
    ...
    const out = runBash(
      VENDOR_FORK,
      ["--catalog-dir", catalog, "catalog-vendor-fixture"],
      { env: { HOME: path.join(root, "home") } }
    );
    expect(out.exitCode).toBe(0);
    expect(
      fs.existsSync(
        path.join(catalog, "vendor/catalog-vendor-fixture/SKILL.md")
      )
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(catalog, "upstream/catalog-vendor-fixture/SKILL.md")
      )
    ).toBe(true);
  });
  ...
  test("sync は selected catalog の upstream を更新する", () => {
    ...
    const out = runBash(VENDOR_SYNC, ["--catalog-dir", catalog], {
      env: { MY_SKILLS_ADD_BIN: stub, MY_SKILLS_TRASH_BIN: trashStub },
    });
    expect(out.exitCode).toBe(0);
    expect(fs.readFileSync(path.join(upstream, "SKILL.md"), "utf-8")).toBe(
      "new upstream\n"
    );
  });
});
...
describe("skills-add", () => {
  test("filter / ignore / fallback / link / no-commit の bash オーケストレーション", () => {
    ...
    fs.writeFileSync(
      path.join(catalog, ".skills-ignore.json"),
    ...
    fs.writeFileSync(
      path.join(active, "existing", "SKILL.md"),
      "---\nname: existing\n---\n"
    );
    ...
    expect(
      Object.keys((custom["skills"] as Record<string, unknown>) ?? {})
    ).toContain("local");
    ...
    for (const agentDir of [claude, gemini]) {
      expect(fs.lstatSync(path.join(agentDir, "alpha")).isSymbolicLink()).toBe(
        true
      );
      expect(
        fs.lstatSync(path.join(agentDir, "fallback")).isSymbolicLink()
      ).toBe(true);
      expect(fs.realpathSync(path.join(agentDir, "alpha"))).toBe(
        fs.realpathSync(path.join(active, "alpha"))
      );
      expect(fs.realpathSync(path.join(agentDir, "fallback"))).toBe(
        fs.realpathSync(path.join(active, "fallback"))
      );
    }
  ...
  test("同一 source の再 add は skip する", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mgmt-add-same-"));
    const repo = path.join(tmp, "repo");
    const clone = Bun.spawnSync([
      "git",
      "clone",
    ...
    fs.mkdirSync(binDir);
    const skillsStub = path.join(binDir, "skills-stub");
    fs.writeFileSync(skillsStub, "#!/usr/bin/env bash\nexit 0\n");
    fs.chmodSync(skillsStub, 0o755);

    const result = runBash(
      path.join(repo, ".agents/skills/skills-add/scripts/skills-add"),
      ["owner/repo", "--skill", "search", "--no-commit"],
      {
        cwd: repo,
        env: {
          ...(process.env as Record<string, string>),
    ...
    expect(lock.external["owner--search"]).toBeUndefined();
    expect(lock.external["search"]).toEqual({
      source: "owner/repo",
      sourceUrl: "https://github.com/owner/repo.git",
      skillPath: "skills/search/SKILL.md",
    });
...
`
    ...
    fs.writeFileSync(
      path.join(root, "candidates.json"),
  ...
  test("an unfiltered install never passes a colliding name to the skills CLI", () => {
    const f = fixture();
    ...
    fs.writeFileSync(
      f.stub,
      `#!/bin/bash\necho "$@" > '${argsFile}'\nmkdir -p "$MY_SKILLS_ACTIVE_DIR/beta"\n`
    );
    expect(f.runUnfiltered().exitCode).toBe(0);
    const args = fs.readFileSync(argsFile, "utf8");
    expect(args).toContain("--skill beta");
    expect(args).not.toContain("--skill alpha");
