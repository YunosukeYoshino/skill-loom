...
function ignoredSets(
  lock: Record<string, unknown>,
  catalogRoot: string
): Set<string> {
  const lockIgnored = lock["ignored"] as unknown[] | undefined;
  const fileIgnored = readJson(path.join(catalogRoot, ".skills-ignore.json"))
    ?.ignore as unknown[] | undefined;
...
function main(): void {
  ...
  const external = lock["external"] as
  ...
  const notInLock = [...updated]
    .filter((n) => !externalNames.includes(n) && !ignored.has(n))
    .sort();
  ...
  if (unmanaged.length > 0) {
    ...
    process.stdout.write(
      "  /skills-add <owner/repo> で lock.json に追加するか\n"
    );
    process.stdout.write(
      "  .skills-ignore.json に追加して明示的に無視してください。\n"
    );
  } else {
