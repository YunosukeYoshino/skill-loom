...
function getManagedSets(lock: Record<string, unknown>): {
  external: Set<string>;
  custom: Set<string>;
  vendor: Set<string>;
  lockIgnored: Set<string>;
} {
  const external = lock["external"] as Record<string, unknown> | undefined;
  const custom =
    (lock["custom"] as { skills?: Record<string, unknown> } | undefined)
      ?.skills ?? {};
  const vendor = lock["vendor"] as Record<string, unknown> | undefined;
  const lockIgnored = lock["ignored"];
  const toSet = (obj: Record<string, unknown> | undefined): Set<string> =>
    new Set(obj ? Object.keys(obj) : []);
  const arrToSet = (arr: unknown): Set<string> =>
    Array.isArray(arr)
...
function getFileIgnored(repoRoot: string): Set<string> {
  ...
  if (
    data &&
    !Array.isArray(data) &&
    Array.isArray((data as Record<string, unknown>).ignore)
  ) {
    return new Set(
      ((data as Record<string, unknown>).ignore as unknown[]).filter(
...
function getInstalledSkills(): Set<string> {
  ...
  for (const entry of fs.readdirSync(GLOBAL_SKILLS_DIR, {
    withFileTypes: true,
  })) {
...
function getGlobalLockSkills(): Record<string, Record<string, unknown>> {
  ...
  if (
    data &&
    !Array.isArray(data) &&
    typeof (data as Record<string, unknown>).skills === "object"
  ) {
    return (data as Record<string, unknown>).skills as Record<
      string,
...
export function isCustomSkill(
  skillName: string,
  repoRoot: string,
  globalLock: Record<string, Record<string, unknown>>,
  customRepo: string
): boolean {
...
export function buildCustomEntry(
  skillName: string,
  repoRoot: string,
  _globalLock: Record<string, Record<string, unknown>>
): { repoPath: string; category: string } | null {
  ...
  if (
    ...
    fs.existsSync(path.join(altPath, "SKILL.md"))
  ) {
...
function buildExternalEntry(
  ...
  if (entry["source"] === undefined || entry["skillPath"] === undefined)
    return null;
  return entry;
}
...
function registerSkills(
  ...
  skillsToRegister: Record<string, Entry>,
  mode: "custom" | "external"
): {
  added: { name: string; entry: Entry }[];
  ...
  const customSkills =
    (custom["skills"] as Record<string, unknown> | undefined) ?? {};
  const external =
    (lock["external"] as Record<string, unknown> | undefined) ?? {};
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--dry-run") args.dryRun = true;
      ...
      const mode = argv[++i];
      if (mode === "auto" || mode === "custom" || mode === "external")
        args.mode = mode;
    }
  }
  return args;
...
export function main(argv: string[]): void {
  ...
  const customRepo = ((lock["custom"] as { repo?: string } | undefined)?.repo ??
    "") as string;
  ...
  let unmanaged = new Set(
    [...installed].filter((n) => !managed.has(n) && !ignored.has(n))
  );
  ...
  if (args.skills) {
    ...
    if (unmanaged.size !== targetSkills.size) {
      const notFound = new Set(
        [...targetSkills].filter((n) => !unmanaged.has(n))
      );
      const alreadyManaged = new Set(
        [...targetSkills].filter((n) => managed.has(n))
      );
      if (alreadyManaged.size > 0) {
        process.stdout.write(
          `⚠️  Already managed (skipped): ${[...alreadyManaged].sort().join(", ")}\n`
        );
      }
      const notExisting = new Set(
        [...notFound].filter((n) => !alreadyManaged.has(n))
      );
      if (notExisting.size > 0) {
        process.stdout.write(
          `⚠️  Not found in installed skills: ${[...notExisting].sort().join(", ")}\n`
        );
      }
    }
  }
  ...
  for (const name of [...unmanaged].sort()) {
    ...
    if (isCustom) {
      ...
      if (entry === null) {
        process.stdout.write(
          `⚠️  Cannot locate repo source for custom skill '${name}' — skipping\n`
        );
        continue;
      }
      ...
      if (entry === null) {
        process.stdout.write(
          `⚠️  Cannot determine source for '${name}' — skipping\n`
        );
        process.stdout.write(
          "   Register manually: /skills-add <owner/repo>\n"
        );
        continue;
      }
  ...
  updateInventoryLock(lockPath, (freshLock) => {
    ...
    if (Object.keys(externalSkills).length > 0) {
      results["external"] = registerSkills(
        freshLock,
        externalSkills,
        "external"
      );
  ...
  for (const [mode, result] of Object.entries(results)) {
    process.stdout.write(
      `  ${mode}: ${result.added.length} added, ${result.skipped.length} skipped\n`
    );
  }
}
