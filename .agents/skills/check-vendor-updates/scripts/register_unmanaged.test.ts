...
describe("is_custom_skill", () => {
  ...
  test("global lock source が catalog repo と完全一致すれば custom と判定", () => {
    ...
    expect(
      isCustomSkill("catalog-skill", dir, globalLock as never, "owner/catalog")
    ).toBe(true);
  });
  ...
  test("catalog repo の接頭辞一致だけでは custom と判定しない", () => {
    ...
    expect(
      isCustomSkill("fork-skill", dir, globalLock as never, "owner/catalog")
    ).toBe(false);
  });
});
  ...
  test("selected catalog は legacy repo root に優先する", () => {
    ...
    try {
      ...
      if (prevCatalog !== undefined)
        process.env.MY_SKILLS_CATALOG_DIR = prevCatalog;
