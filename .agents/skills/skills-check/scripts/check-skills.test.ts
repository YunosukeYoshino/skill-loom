...
import { classify, loadIgnore, loadLockIgnored, repoDefaults } from "./check-skills"
...
describe("repoDefaults", () => {
  ...
  test("MY_SKILLS_CATALOG_DIR を優先する", () => {
    ...
    try {
      const defaults = repoDefaults()
      expect(defaults.lockFile).toBe(path.resolve(catalog) + "/skills.lock.json")
      expect(defaults.ignoreFile).toBe(path.resolve(catalog) + "/.skills-ignore.json")
    } finally {
      if (prev !== undefined) process.env.MY_SKILLS_CATALOG_DIR = prev
...
describe("classify", () => {
  test("custom / external / ignored / unmanaged を分類する", () => {
    const lock = {
      custom: { repo: "owner/catalog", skills: { alpha: { repoPath: "skills/a/alpha", category: "a" } } },
      external: { beta: { source: "owner/repo" } },
      vendor: {},
    }
    const result = classify(lock, new Set(["superpowers"]), new Set(["alpha", "beta", "superpowers", "unknown"]), new Set(["alpha", "unknown"]))
  ...
  test("lock の ignored 配列は ignored として扱う", () => {
    const lock = { custom: { repo: "owner/catalog", skills: {} }, external: {}, vendor: {}, ignored: ["plugin-skill"] }
    const result = classify(lock, new Set(), new Set(["plugin-skill"]), new Set())
...
describe("loadIgnore / loadLockIgnored", () => {
  ...
  test("load_lock_ignored は lock の ignored 配列を返す", () => {
    expect(loadLockIgnored({ ignored: ["plugin-skill"] })).toEqual(new Set(["plugin-skill"]))
  })
})