...
export function repoDefaults(): RepoDefaults {
  ...
  if (selectedCatalog) {
    repoRoot = path.resolve(
      selectedCatalog.replace(/^~(?=$|\/)/, os.homedir())
    );
  } else {
...
export function loadLock(lockFile: string): Record<string, unknown> {
  ...
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    process.stderr.write(
      `Error: Lock file is not a JSON object: ${lockFile}\n`
    );
    process.exit(1);
  }
...
export function loadIgnore(ignoreFile: string): Set<string> {
  ...
  const data = JSON.parse(fs.readFileSync(ignoreFile, "utf-8")) as {
    ignore?: unknown;
  };
...
export function buildManagedSkills(
  lock: Record<string, unknown>
): Record<string, ManagedEntry> {
  const managed: Record<string, ManagedEntry> = {};
  const custom =
    (lock["custom"] as
      | { repo?: unknown; skills?: Record<string, unknown> }
      | undefined) ?? {};
  const external =
    (lock["external"] as Record<string, { source?: unknown }> | undefined) ??
    {};
...
function printResults(result: ClassifyResult): void {
  ...
  const extTotal = Object.values(result.external).reduce(
    (sum, list) => sum + list.length,
    0
  );
  const unmanagedAll = new Set([
    ...result.unmanagedAgents,
    ...result.unmanagedClaude,
  ]);

  process.stdout.write("=== CUSTOM (Catalog) ===\n");
  let issuesCustom = 0;
  for (const [name, state] of Object.entries(result.custom).sort(([a], [b]) =>
    a.localeCompare(b)
  )) {
  ...
  if (result.ignored.size > 0) {
    process.stdout.write(
      `=== IGNORED (plugin-managed, ${result.ignored.size}) ===\n`
    );
...
function main(): void {
  ...
  const ignoreList = new Set([
    ...loadIgnore(ignoreFile),
    ...loadLockIgnored(lock),
  ]);
